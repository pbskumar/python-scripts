...
#	Only a maximum of ~3200 recent tweets can be extracted from a user timeline
...
class MongoServer:
  ...
  def write_doc_to_collection(self, db_name, collection_name, json_data):
    ...
    # Tries inserting document into the collection. If server connection is terminated, the connection is reset and tries to insert again
    try:
...
def retrieve_user_tweet(cursor, api, user_id):
  """
  ...
  while True:
    ...
    try:
      ...
      if tweet != '':
        ...
        cursor = tweepy.Cursor(api.user_timeline, id=user_id, max_id=max_tweet_id).items()
      time.sleep(15 * 60)
      ...
      print("Cursor Iteration of " + user_id + " Complete. No more tweets available")
      return


def user_tweets(user_id, api, db_reference, db_conf_list):
  """
  ...
  """
  print("Extracting tweets from user timeline")
  for tweet in retrieve_user_tweet(tweepy.Cursor(api.user_timeline, id=user_id).items(), api, user_id):
...
class DataRetrievalThread(threading.Thread):
  ...
  def __init__(self, api, db_reference, db_conf_list, thread_id):
    """
    Initializes the DataRetrievalThread class
    :param api: Tweepy API Handle
    :param db_reference: MongoServer reference
    :param db_conf_list: List of target database name and collection name.
    :param thread_id: ID of the thead for identification
    """
...
if __name__ == '__main__':
  ...
  # Use one of the following commented blocks to extract tweets
